    ...
    return (
      ...
      >
        <ProForm.Group>
          ...
          <ProFormSelect
            ...
            onChange={(value: any) => {
              ...
              form.setFieldsValue({
                ...
                activity_unit: typeItem?.unit.split('/')[1],
                emission_factor_unit: `t-CO2e/${typeItem?.unit.split('/')[1]}`,
                counterparty_nm: "",
                energy_cls: 0,
              })
            }}
          ...
          />
          {kindCdKey === 'SCOPE2ENERGYTYPE1' && <ProFormSelect
            width="xl"
            name="counterparty_nm"
            label="相手先"
            placeholder="エネルギーの購入先・プラン名"
            options={scopeData?.buy_from_lists?.map(i => ({ label: i.name, value: i.name }))}
            onChange={(value: any) => {
              const buyFromItem = scopeData?.buy_from_lists?.find(i => i.name === value);
              form.setFieldsValue({ emission_factor: buyFromItem?.emission_value })
            }}
            rules={[{ required: true, message: '必須' }]}
            showSearch
          />}
          {kindCdKey === 'SCOPE2ENERGYTYPE2' && <ProFormSelect
            width="xl"
            name="energy_cls"
            label="区分"
            options={[{ label: "蒸気(産業用のものは除く)温水、冷水　排出係数：0.060", value: 1 }, { label: "産業用蒸気 排出係数：0.057", value: 2 }]}
            onChange={(value: any) => {
              form.setFieldsValue({ emission_factor: [0, 0.060, 0.057][value] })
            }}
            rules={[{ required: true, message: '必須' }]}
          />}
          <ProFormText
            ...
            rules={[{ required: true, message: '必須' }]}
          />
          {kindCdKey === 'SCOPE2ENERGYTYPE2' && <ProFormText
            width="xl"
            name="counterparty_nm"
            label="相手先"
            placeholder="エネルギーの購入先・プラン名"
...
const UpdateModal: React.FC<{
  ...
  ({ adminInfo, organizationMstsData, scopeData, industryAttributeInfoDetailsRes, itemData }) => {
    ...
    const kindCdKey = Form.useWatch('kind_cd', form);
    const factorUnit = Form.useWatch('emission_factor_unit', form);
    const { _id, fiscal_y, monthly, organization_id, counterparty_nm, kind_cd, emission_factor, activity_energy, energy_cls, emission_factor_unit, activity_unit } = itemData;
    return (
      <ModalForm<{
        monthly: string;
        organization_id: string;
      ...
      >
        ...
        <ProFormItem name="kind_type_nm" />
        <ProFormItem name="activity_unit" initialValue={activity_unit} />
        <ProFormItem name="emission_factor_unit" initialValue={emission_factor_unit} />
  ...
  const handleUpload = async (info: any) => {
    if (info.file.status === 'done') {
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        const energy_cls = String(line["区分"]);

        if (Object.keys(currentError).length === 1) {
          const kind_cd = currentScopeData?.id;
          let emission_factor = 0;
          if (kind_type_nm === '電気') {
            emission_factor = scopeData?.buy_from_lists?.find(item => item.name === counterparty_nm)?.emission_value || 0
          }else if(kind_type_nm === '熱') {
            emission_factor = { "蒸気(産業用のものは除く)温水、冷水": 0.060, "産業用蒸気": 0.057 }[energy_cls] || 0;
          }
          ...
          payload.push({
            ...
            emission_factor,
            activity_energy,
            energy_cls: { "蒸気(産業用のものは除く)温水、冷水": 1, "産業用蒸気": 2 }[energy_cls],
      ...
      if (errorList.length > 0) {
        const header = ["行番号", "月", "事業/事業所", "相手先", "種類", "活動量（エネルギー）"];
...
const TemplateDownload: React.FC<any> = () => {
  const header = ["月", "事業", "事業所", "相手先", "種類", "区分", "活動量（エネルギー）"];
  const wscols = header.map(i => ({ width: 25 }));
  const handleDownloadClick = () => {
