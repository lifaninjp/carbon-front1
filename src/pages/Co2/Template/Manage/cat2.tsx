      ...
      >
        <ProForm.Group>
          ...
          <ProFormSelect
            ...
            rules={[{ required: true, message: '必須' }]}
            showSearch
            extra="t-CO2e/kWh に基づいて計算してください"
          />
          <ProFormText
            width="xl"
...
const UpdateModal: React.FC<{
  ...
  ({ adminInfo, organizationMstsData, scopeData, industryAttributeInfoDetailsRes, itemData }) => {
    ...
    const { _id, fiscal_y, monthly, organization_id, counterparty_nm, kind_cd, emission_factor, activity_energy } = itemData;
    return (
      <ModalForm<{
        monthly: string;
        organization_id: string;
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        if (Object.keys(currentError).length === 1) {
          ...
          payload.push({
            ...
            activity_energy,
            activity_unit: "kWh",
            emission_factor_unit: "t-CO2e/kWh",
            ghg_emission: emissionFactor.times(activityAmount).toNumber(),
          })
        } else {
...
const TemplateDownload: React.FC<any> = () => {
  const header = ["月", "事業", "事業所", "相手先", "種類", "活動量（エネルギー）"];
  const wscols = header.map(i => ({ width: 25 }));
  const handleDownloadClick = () => {
