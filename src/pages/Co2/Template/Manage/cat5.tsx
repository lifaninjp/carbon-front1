  ...
  return (
    <ModalForm<{
      ...
      onFinish={async (values) => {
        ...
        let activityAmount = !kindCdKey.includes('ETE') ? new Decimal(values.activity_people * values.activity_period) : new Decimal(values.activity_amount);
    ...
    >
      <ProForm.Group>
        ...
        <ProFormSelect
          ...
          onChange={(value: any) => {
            ...
            form.setFieldsValue({
              ...
              activity_unit: !value.includes('ETE') ? '人・日' : '百万円',
              emission_factor_unit: !value.includes('ETE') ? "t-CO2e/人・日" : "t-CO2e/百万円"
            });
          }}
          rules={[{ required: true, message: '必須' }]}
          showSearch
          extra={kindCdKey ? `t-CO2e/${kindCdKey.includes('ETE') ? "百万円" : "人・日"} に基づいて計算してください` : ''}
        />
        <ProFormText
          width="xl"
          ...
          rules={[{ required: true, message: '必須' }]}
        />
        {kindCdKey && !kindCdKey?.includes('ETE') && <><ProFormText
          width="xl"
        ...
        <ProFormText
          ...
          rules={[{ required: true, message: '必須' }]}
        /></>}
        {kindCdKey?.includes('ETE') && <><ProFormText
          width="xl"
          name="item_nm"
          label="項目名"
          placeholder="交通機関名"
        ...
        <ProFormSelect
          ...
          onChange={(value: any) => {
            ...
            form.setFieldsValue({
              kind_type_nm: typeItem?.name,
              emission_factor: typeItem?.emission_value,
              activity_unit: value.slice(0, 3) === 'ETE' ? '百万円' : '人・日',
              emission_factor_unit: value.slice(0, 3) === 'ETE' ? "t-CO2e/百万円"  : "t-CO2e/人・日"
            });
          }}
          rules={[{ required: true, message: '必須' }]}
  ...
  const handleUpload = async (info: any) => {
    if (info.file.status === 'done') {
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        const item_nm = line['項目名'] ? String(line['項目名']) : '';
