          ...
          <ProFormSelect
            ...
            onChange={(value: string) => {
              ...
              form.setFieldsValue({
                ...
                activity_unit: { "Scope3Cat15Bond": "口", "Scope3Cat15Stock": "株", "Scope3Cat15Project": "円" }[value],
                emission_factor_unit: { "Scope3Cat15Bond": "t-CO2e/口", "Scope3Cat15Stock": "t-CO2e/株", "Scope3Cat15Project": "t-CO2e/円" }[value]
              })
            }}
            rules={[{ required: true, message: '必須' }]}
            showSearch
            extra={kindCdKey ? `t-CO2e/${{ "Scope3Cat15Bond": "口", "Scope3Cat15Stock": "株", "Scope3Cat15Project": "円" }[kindCdKey]} に基づいて計算してください` : ''}
          />
          <ProFormText
            width="xl"
...
const UpdateModal: React.FC<{
  ...
  ({ adminInfo, organizationMstsData, scopeData, industryAttributeInfoDetailsRes, itemData }) => {
    ...
    const { _id, fiscal_y, monthly, organization_id, counterparty_nm, kind_cd, emission_factor, activity_amount } = itemData;
    const kindCdKey = Form.useWatch('kind_cd', form);
    return (
      <ModalForm<{
        monthly: string;
  ...
  const handleUpload = async (info: any) => {
    if (info.file.status === 'done') {
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        const kind_type_nm: string = line["種類"];
        ...
        const emission_factor = Number(line["排出係数"]) || 0;
        ...
        if (Object.keys(currentError).length === 1) {
          ...
          payload.push({
            ...
            emission_factor,
            activity_amount,
            activity_unit: { "債券": "口", "株": "株", "プロジェクト": "円" }[kind_type_nm],
            emission_factor_unit: { "債券": "t-CO2e/口", "株": "t-CO2e/株", "プロジェクト": "t-CO2e/円" }[kind_type_nm],
            ghg_emission: emissionFactor.times(activityAmount).toNumber(),
          })
        } else {
      ...
      if (errorList.length > 0) {
        const header = ["行番号", "月", "事業/事業所", "相手先", "種類", "活動量（金額）【百万円】"];
...
const TemplateDownload: React.FC<any> = () => {
  const header = ["月", "事業", "事業所", "相手先", "種類", "活動量（金額）【百万円】", "排出係数"];
  const wscols = header.map(i => ({ width: 25 }));
  const handleDownloadClick = () => {
