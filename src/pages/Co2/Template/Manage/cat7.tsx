    ...
    return (
      <ModalForm<{
        ...
        onFinish={async (values) => {
          ...
          const activityAmount = new Decimal(kindCdKey.includes('EBM') ? values.activity_energy : values.activity_area * values.activity_period)
      ...
      >
        <ProForm.Group>
          ...
          <ProFormSelect
            ...
            options={[
              {
                label: <span>エネルギー消費把握可能</span>,
                value: '1',
                options: scopeData?.type_lists
                  ?.filter(i => i.id.includes('EBM'))
                  .map((i) => ({ label: i.name, value: i.id })),
              },
              {
                label: <span>エネルギー消費把握不可</span>,
                value: '2',
                options: scopeData?.type_lists
                  ?.filter(i => i.id.includes('EBA'))
                  .map((i) => ({ label: i.name, value: i.id })),
              },
            ]}
            onChange={(value: any) => {
              ...
              form.setFieldsValue({
                ...
                activity_unit: value.includes('EBM') ? 'kWh' : 'm2',
                emission_factor_unit: value.includes('EBM') ? "t-CO2e/kWh" : "t-CO2e/m2"
              })
            }}
            rules={[{ required: true, message: '必須' }]}
            showSearch
            extra={kindCdKey ? `t-CO2e/${kindCdKey.includes('EBM') ? "kWh" : "m2"} に基づいて計算してください` : ''}
          />
          <ProFormText
            width="xl"
            ...
            rules={[{ required: true, message: '必須' }]}
          />
          {kindCdKey?.includes('EBM') && <ProFormText
            width="xl"
            ...
            rules={[{ required: true, message: '必須' }]}
          />}
          {kindCdKey?.includes('EBA') && <><ProFormText
            width="xl"
            name="activity_area"
            label="活動量（面積）"
            placeholder="床面積[m2]"
...
const UpdateModal: React.FC<{
  ...
  ({ adminInfo, organizationMstsData, scopeData, industryAttributeInfoDetailsRes, itemData }) => {
    ...
    const { _id, fiscal_y, monthly, organization_id, counterparty_nm, item_nm, kind_cd, emission_factor, activity_energy, activity_area, activity_period } = itemData;
    const kindCdKey = Form.useWatch('kind_cd', form);
    return (
      <ModalForm<{
        monthly: string;
        ...
        submitTimeout={1000}
        onFinish={async (values) => {
          if (kindCdKey.includes('EBM')) {
            values.activity_area = 0;
            values.activity_period = 0;
          } else {
            values.activity_energy = 0;
          }
          const emissionFactor = new Decimal(values.emission_factor);
          const activityAmount = new Decimal(kindCdKey.includes('EBM') ? values.activity_energy : values.activity_area * values.activity_period)
  ...
  const handleUpload = async (info: any) => {
    if (info.file.status === 'done') {
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        if (activity_energy === 0 && activity_area === 0 && activity_period === 0) {
        ...
        if (Object.keys(currentError).length === 1) {
          ...
          if (activity_energy > 0) {
            activityAmount = new Decimal(activity_energy);
          } else {
          ...
          payload.push({
            ...
            activity_area,
            activity_period,
            activity_unit: `${activity_energy > 0 ? "kWh" : "m2"}`,
            emission_factor_unit: `${activity_energy > 0 ? "t-CO2e/kWh" : "t-CO2e/m2"}`,
            ghg_emission: emissionFactor.times(activityAmount).toNumber(),
          })
        } else {
      ...
      if (errorList.length > 0) {
        const header = ["行番号", "月", "事業/事業所", "相手先", "種類", "活動量（エネルギー）", "活動量（面積）", "活動量（期間）"];
...
const TemplateDownload: React.FC<any> = () => {
  const header = ["月", "事業", "事業所", "相手先", "項目名", "種類", "活動量（エネルギー）", "活動量（面積）", "活動量（期間）"];
  const wscols = header.map(i => ({ width: 25 }));
  const handleDownloadClick = () => {
