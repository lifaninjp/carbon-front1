    ...
    return (
      <ModalForm<{
        ...
        onFinish={async (values) => {
          ...
          const activityAmount = new Decimal(values.activity_weight)
      ...
      >
        <ProForm.Group>
          ...
          <ProFormText
            ...
            label="活動量（重量）"
            placeholder="産業廃棄物発生量[t]"
            rules={[{ required: true, message: '必須' }]}
          />
        </ProForm.Group>
        <ProFormItem name="kind_type_nm" />
        <ProFormItem name="activity_unit" initialValue="t" />
        <ProFormItem name="emission_factor_unit" initialValue="t-CO2e/t" />
...
const UpdateModal: React.FC<{
  ...
  ({ adminInfo, organizationMstsData, scopeData, industryAttributeInfoDetailsRes, itemData }) => {
    ...
    const { _id, fiscal_y, monthly, organization_id, item_nm, kind_cd, emission_factor, activity_weight } = itemData;
    return (
      <ModalForm<{
        monthly: string;
        organization_id: string;
    ...
    if (info.file.status === 'done') {
      ...
      for (let i = 0; i < sheetData.length; i++) {
        ...
        if (Object.keys(currentError).length === 1) {
          ...
          const activityAmount = new Decimal(activity_weight)
          payload.push({
            create_prg_id: adminInfo?.manager_id,
            industry_info_id,
            company_id,
      ...
      if (errorList.length > 0) {
        const header = ["行番号", "月", "事業/事業所", "相手先", "項目名", "種類", "活動量（金額）【t】"];
...
const TemplateDownload: React.FC<any> = () => {
  const header = ["月", "事業", "事業所", "相手先", "項目名", "種類", "活動量（金額）【t】"];
  const wscols = header.map(i => ({ width: 25 }));
  const handleDownloadClick = () => {
